...
function metrics() { document.querySelector('#metrics').textContent = `Observed requests: ${requests} · scenario: ${scenario}`; }
...
setInterval(async () => { const response = await fetch('/metrics'); requests = (await response.json()).requests; metrics(); }, 800);
