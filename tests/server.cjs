...
http.createServer((req,res) => {
  ...
  let file;
  if (/^\/demo_profile(?:\/followers_you_follow)?$/.test(url.pathname)) { requests=0; scenario=url.searchParams.get('scenario') ?? 'normal'; file=scenario==='demo'?'tests/demo.html':'tests/browser.html'; }
  else if (['/endpoint.js','/core.js','/content.js','/popup.html','/popup.css','/tests/browser.js','/tests/demo.js','/docs/x-open.jpg'].includes(url.pathname)) file=url.pathname.slice(1);
